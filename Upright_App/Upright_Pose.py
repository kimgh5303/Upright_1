...
def camStart():
    ...
    try:
        ...
        while running:                                              #루프 반복
            ...
            # 이미지를 JPEG로 인코딩
            hasFrame, frame_data = cv.imencode('.jpg', frame, [cv.IMWRITE_JPEG_QUALITY, 90])
            # 인코딩된 이미지 데이터를 직렬화하여 전송
            frame_data_serialized = pickle.dumps(frame_data)
            print("전송 프레임 크기 : {} bytes".format(len(frame_data_serialized)))
            client_socket.sendall(struct.pack(">L", len(frame_data_serialized)) + frame_data_serialized)
            ...
            pose_data = b""
            while len(pose_data) < data_size:
                pose_data += client_socket.recv(4096)
            # 역직렬화된 포즈 데이터 처리
            points = pickle.loads(pose_data)
